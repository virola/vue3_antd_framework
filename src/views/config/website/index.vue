      ...
      <a-tabs v-model:activeKey="activeKey" @change="handleChange">
        <a-tab-pane v-for="(item,index) in configList" :key="index">
          ...
          <a-form
            ...
            v-for="(subItem,val) in item.dataList" :key="val">
            <!-- 单行文本输入框 -->
            <a-form-item
            ...
            <a-form-item
              ...
              :label-col="{sm: {span: 5}, xs: {span: 6}}"
              :wrapper-col="{sm: {span: 19}, xs: {span: 18}}">
              <uploadImage :limit="1" :updDir="updDir" v-model:value="form[subItem.code]"/>
            </a-form-item>
            <!-- 多图上传 -->
            <a-form-item
              ...
              :label-col="{sm: {span: 5}, xs: {span: 6}}"
              :wrapper-col="{sm: {span: 19}, xs: {span: 18}}">
              <uploadImage :limit="10" :isMultiple="true" :updDir="updDir" v-model:value="form[subItem.code]"/>
            </a-form-item>
            <!-- 日期选择 -->
            <a-form-item
...
export default {
    ...
    initEditor() {
      return {
        ...
        images_upload_handler: (blobInfo, success, error) => {
          ...
          this.$http.post('/system/upload/uploadImage/' + this.updDir, formData).then(res => {
            if (res.data.code == 0) {
  ...
  methods: {
     ...
     */
     getConfigInfo() {
      this.$http.get('/system/configweb/index').then(res => {
        if (res.data.code === 0) {
    ...
    save() {
      this.loading = true;
      this.$http.put('/system/configweb/edit', this.form).then(res => {
        this.loading = false;
        if (res.data.code === 0) {
    ...
    handleChange(activeKey) {
      let itemList = this.configList[activeKey]['dataList'];
        let itemArr = {};
        itemList.forEach(item => {
          if (item.valueList) {
            itemArr[item.code] = item.valueList;
