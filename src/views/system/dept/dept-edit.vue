  ...
  data() {
    return {
      ...
      rules: {
        ...
        fullname: [
          {required: true, type: 'string', message: '请输入部门全称', trigger: 'blur'}
        ],
        code: [
          {required: true, type: 'string', message: '请输入部门编码', trigger: 'blur'}
        ],
        type: [
          {required: true, type: 'number', message: '请选择部门类型', trigger: 'blur'}
        ],
        sort: [
  ...
  methods: {
    ...
    save() {
      this.$refs.form.validate().then(() => {
        this.loading = true;
        this.$http[this.isUpdate ? 'put' : 'post'](this.isUpdate ? '/system/dept/edit' : '/system/dept/add',
