...
export default {
  ...
  methods: {
    ...
    query() {
      ...
      this.$http.get('/system/role/getPermissionList/' + this.data.id).then(res => {
    ...
    save() {
      ...
      this.$http.post('/system/role/savePermission', {roleId: this.data.id, menuIds: Array.from(ids)}).then(res => {
        this.loading = false;
        if (res.data.code === 0) {
