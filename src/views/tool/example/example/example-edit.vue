...
<template>
  <a-modal
    ...
    <a-form
      ...
      <a-form-item label="是否VIP" name="isVip">
        <a-radio-group
          v-model:value="form.isVip">
    ...
    data() {
      return {
        ...
        rules: {
          ...
          isVip: [
            {required: true, message: '请输入是否VIP：1是 2否', type: 'string', trigger: 'blur'}
          ],
          planTime: [
            {required: true, message: '请输入计划时间', type: 'string', trigger: 'blur'}
          ],
          status: [
    ...
    methods: {
      ...
      save() {
        this.$refs.form.validate().then(() => {
          this.loading = true;
          this.$http[this.isUpdate ? 'put' : 'post'](this.form.id ? '/system/example/edit' : '/system/example/add', this.form).then(res => {
            this.loading = false;
            if (res.data.code === 0) {
